...
class ChatViewModel(
     ...
     */
    fun sendImages(visiteId: String, uris: List<android.net.Uri>, context: android.content.Context) {
        ...
        viewModelScope.launch {
            ...
            try {
                    ...
                    try {
                        ...
                        if (bytes != null) {
                            val requestFile = okhttp3.RequestBody.create(
                                "image/*".okhttp3.MediaType.Companion.toMediaTypeOrNull(),
                                bytes
                            )
                            okhttp3.MultipartBody.Part.createFormData("images", "image_${System.currentTimeMillis()}.jpg", requestFile)
                        } else null
                    } catch (e: Exception) {
                        Log.e("ChatViewModel", "Erreur lors de la lecture de l'image: ${e.message}")
                        null
                    }
                }
                ...
                if (imageParts.isNotEmpty()) {
                    ...
                    _state.update { it.copy(isSending = false, error = "Impossible de traiter les images") }
                }
            } catch (error: Exception) {
                _state.update { 
                    it.copy(
