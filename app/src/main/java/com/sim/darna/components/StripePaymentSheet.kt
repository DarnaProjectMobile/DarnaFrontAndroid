...
fun StripePaymentBottomSheet(
    ...
    // Mettre à jour le callback du PaymentSheetManager et s'assurer qu'il est initialisé
    LaunchedEffect(Unit) {
        if (activity != null) {
            // S'assurer que PaymentSheetManager est initialisé (fallback si MainActivity.onCreate n'a pas été appelé)
            PaymentSheetManager.initialize(activity)
