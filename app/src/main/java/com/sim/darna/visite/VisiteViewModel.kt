...
class VisiteViewModel(
    ...
    private fun extractShortMessage(errorBody: String): String {
        if (errorBody.length <= 100 && !errorBody.contains("{")) return errorBody
        
        try {
            // Try to parse as generic JSON to handle both string and array messages
            val jsonObject = org.json.JSONObject(errorBody)
            if (jsonObject.has("message")) {
                val messageObj = jsonObject.get("message")
                return when (messageObj) {
                    is org.json.JSONArray -> {
                        // If it's an array, take the first item
                        if (messageObj.length() > 0) messageObj.getString(0) else "Erreur de validation"
                    }
                    is String -> messageObj
                    else -> messageObj.toString()
                }
            }
        } catch (e: Exception) {
            // Fallback to regex if JSONObject parsing fails (though unlikely on Android if org.json is available)
            // Regex for array: "message":\["([^"]+)"
            val arrayMatch = Regex(""""message"\s*:\s*\["([^"]+)"""").find(errorBody)
            if (arrayMatch != null) {
